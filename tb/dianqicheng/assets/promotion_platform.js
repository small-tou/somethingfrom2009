...
(function(B) {
        ...
        utils = {
            JSON: {
                parse: function(data) {
                    ...
                    }catch (e) { alert(CONST['JSON_PARSE_ERROR']);}
                    return ret;
                },
            ...
            size: function(o) {
                var l = 0, k;
                for (k in o) { o.hasOwnProperty(k) && (l += 1); }
                return l;
            },
            isEmpty: function(o) {
    ...
    Lang.augmentObject(Model.prototype, {
        _get: function(o, s) {
            ...
            for (; i < len; i += 1) {
                ...
                ret = data && data[key];
                if (ret !== undefined) {data = ret;} else {ret = null;break;}
            }
            return ret;
        },
        ...
        _delete: function(o, s) {
            ...
            if (Lang.isObject(data)) {try {delete data[key];}catch (e) {}}
        },
        query: function(s) {
    ...
    OrderModel = function(o, uid, config) { // {{{
        ...
        utils.each(eventsList, function(item) {self.createEvent(item);});
        self.initialize();
    };
    ...
    Lang.augmentObject(OrderModel.prototype, { // {{{
        initialize: function() { // {{{
            ...
            twistList.forEach(function(key){
                self.subscribe(key,function(o){
                    self.calcTwist({'event':(o && o.event) || key});
                });
            });
            ...
            self.subscribe('changeSelected',function(o){
                ...
                o.deliveryChange && self.changePostage({'refresh':true});
                o.bundles && self.changeBundles({'val':o.bundles});

                self.changeBan(o.banlist);
                self.calcTwist({'event':'changeSelected'});
            });

            try { self.changePostage({isInit: true}); }catch (E1) {
                console.log('E1',E1.message);
            }
            try { self.changeSelected({isInit: true}); }catch (E2) {
            ...
            if (!B.Single && !self.getAttr('cross') && !self.getAttr('parent')) {B.Single = self};
        }, // }}}
        _async: function(act) { // {{{
            ...
            asyncQueue = Connect.asyncRequest(method, url, {
                success: function(res) {
                    ...
                    if (jsonData.result === 'success') {
                        ...
                        self.fireEvent('async',{'val':mergeData['orders']});
                    } else {
                        self.fireEvent('promotionFailure');
                    }
                },
        ...
        calculator: function(o) { // {{{
            ...
            if (self.getAttr('parent')) {
                ...
                if (postType === 'cod') {
                    postageFee = self._CODCalc({'refresh':true});
                } else {
                    postageFee = self._postageCalc({'refresh':true});
                };
        ...
        changeSelected: function(o) { // {{{
            var self = this, isInit = o && o.isInit || false, silent = o && o.silent || false, isAsync = o && o.isAsync || false,
                ...
                comparePromo = function() {
                    ...
                    ret = !isInit ? {'original': original, 'current': current, 'stay': stay, 'added': added, 'removed': removed} : {
                                    'original': [], 'current': current, 'stay': [], 'added': current, 'removed': [] };
                ...
                coupleUnit = function() {
                    ...
                    promoAdded.forEach(function(item) {
                        ...
                        if (!unused[0]) {banlist['added'].push(item);}
                    });
                    ...
                    promoRemoved.forEach(function(item) {
                        ...
                        if (unused[0]) {banlist['removed'].push(item);}
                    });
        ...
        changePostage: function(o) { // {{{
            ...
            if (self.getAttr('parent') || self.getAttr('cross')) {return;}

            postType === 'post' ? self._postageCalc({'val':val}) :  self._CODCalc({'val':val,'refresh':refresh});
        ...
        asyncMerge:function(o){ // {{{
            var self = this,
                ...
                correct = function(){
                    ...
                    p.forEach(function(item){
                        if (original.indexOf(item) === -1) {added.push(item)};
                    });

                    added.some(function(item){
                        if(banlist.indexOf(item) !== -1){ret = null;return true;}
                    });
                    return ret;
                };
            ...
            if (bundle || bundles) {
                bundle = correct();
                self.changeSelected({'val':bundle,'bundles':bundles,'averageSum':averageSum,'isAsync':true});
            } else if (averageSum) {
                self.averageSum({'val':averageSum,'isAsync':true});
            };
        } // }}}
    }); // }}}
    ...
    Lang.augmentObject(OrderView.prototype, { // {{{
                ...
                if (model.getAttr('parent') && same) {
                    same.forEach(function(item){
                        model.query('relation->' + item).forEach(function(idx){
                            ...
                            if (m !== model) {
                                m.fireEvent('changeBundles', {event: 'changeBan'});
                            }
                        });
                    });
                } else {
        ...
        renderAmount:function(){
            var amounts=DOM.query(".J_Amount");
            S.each(amounts,function(amount){
        ...
        renderBundles: function(silent) { // {{{
            var self = this, model = self.model, controller = self.controller, config = model.config,
                ...
                inBanList = function(b){
                    ...
                    p.some(function(idx){
                        if(banlist.indexOf(idx) !== -1){ret = true;return true;}
                    });
                    return ret;
                },
                createOptions = function() {
                    ...
                    if (!bundleList || utils.isEmpty(bundleList)) {
                        ...
                        for (k in bundleList) {
                            if (k !== selected && inBanList(k)) {continue;}
            ...
            if(!bundlesCon){return;}
            optionList = createOptions();
            if (optionList) {
                self._setHTML(bundlesCon,Lang.substitute(bundleTemplate, {'options': optionList.join(''), 'uid': model.uid}));
        ...
        renderPostage: function(silent) { // {{{
            var self = this, model = self.model,
                ...
                if(!fareEl || model.getAttr('parent')) {return;}
                ...
                if(postType === 'post') {
                    ...
                    showUnit = {
                        ...
                        select_empty:function(){
                            ...
                            emptySnippet = Lang.substitute(CONST['COD_PROMO_TPL'], {'sum': serviceSum});
                        ...
                        empty:function(){
                            ...
                            if (codFee) {
                                snippetArr = [CONST['NO_POSTAGE_FEE'],Lang.substitute(CONST['COD_PROMO_TPL'], {'sum': codSum})];
        ...
        renderSum: function() { // {{{
            ...
            for (k in elems) {
                ...
                //console.log(k, item);
                if (re.test(k)) {item = (item / 100).toFixed(2);}
                if (elem && item) {
        ...
        renderExt: function(){
            ...
            if (gift) {
                ...
                gift.split(';').forEach(function(s){
                    ...
                    temp.push(Lang.substitute(giftSnippet,{'title':sp[0],'href':sp[1]}));
    ...
    Lang.augmentObject(OrderController.prototype, { // {{{
        ...
        amount: function(o) { // {{{
            ...
            if (!o.force && model.getAttr('amount') === o.val) {return;}
            model.changeAmount({val: o.val, silent: false});
        } // }}}
    }); // }}}
    ...
    B.init = function(config) {
        if (!config) {return;}
        var promoData = config.promoData, orderData = config.orderData, viewsDock = [],
            mergeChunk = function() {
