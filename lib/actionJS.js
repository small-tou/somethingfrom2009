...
UIEvents模块提供对UIEvent类型事件的支持：DOMActivate,DOMFocusIn,DOMFocusOut
         */
        actMouse:function(el,type){
            if (document.createEvent)
            {
                var evObj = document.createEvent("MouseEvents")
                evObj.iniMouseEvent( type, true, false )
        ...
        actUI:function(el,type){
            if (document.createEvent)
            {
                var evObj = document.createEvent("UIEvents")
                evObj.iniUIEvent( type, true, false )
                el.dispatchEvent(evObj)
            }
            else if (document.createEventObject)
            {
    ...
    ACTJS_CASE.prototype={
        ...
        hasElements:function(els){
            for(var i in els){
                if(cssQuery(els[i]).length==0){
                    this.log.push({
                        type:"verify",
    ...
    ACTJS=function(){
        ...
        return {
            create:function(info){
                info.id=cases.length;
                var c=new ACTJS_CASE(info);
                cases.push(c);
                return c;
            }
        }
