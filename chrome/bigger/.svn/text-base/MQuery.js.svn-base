...
/**
 *约定:
 *1.在dom元素中缓存数据的时候一律用 m-data-classcache这种方式命名,第三部分是具体名称,前两个是固定格式
 *例如在toggleClass的时候,将原来的className存储在dom中 用m-data-classcache=",,,"存储
 */
MY.create("Query",function(){
    var D=MY.Dom,E=MY.Event,A=MY.Array,V=MY.Vector
    ...
    Query.prototype={
        ...
        toggleClass:function(className){
            this.each(function(i){
                var j;
                if((j=MY.MQuery(i)).hasClass(className)){
        ...
        anim:function(config,interval){
             this.each(function(i){
                 var anim=new MY.Anim(i,config,interval)
                 anim.animate();
            })
        },
        ajax:function(method1,url1,callback1,postdata){
            var ajax=new MY.Ajax(method1,url1,callback1,postdata);
        },
        _addEvent:function(){
    ...
    //添加jq式的事件支持
    var EventList=[
    "click","blur","mouseover","mousemove","mouseout","mousedown","mouseup","focus","load","abort","change","dblclick","keydown","focus","keypress","keyup","resize","select","submit","unload"
    ]
