        ...
        return {
            ...
            createElement:function(tagName,params,childs){
                ...
                if(params!=undefined){
                    ...
                    if(params.s){
                        MY.Dom.mixinStyle(ele,params.s)
                    }
                }
            ...
            getByClass:function(className, tag, root){
                ...
                for(var i=0,j=eles.length;i<j;i++){
                    if(MY.Dom.hasClass(className,eles[i].className)){
            ...
            getDocumentWidth:function(){
                return _bodyValue("clientWidth")
            },
            getDocumentHeight:function(){
                return _bodyValue("clientHeight")
            },
            getScrollXY:function(){
                var x=document.documentElement.scrollLeft||document.body.scrollLeft;
                var y=document.documentElement.scrollTop||document.body.scrollTop;
                return new MY.Vector(x,y)
            },
            setXY:function(ele,point){
                ...
                return new MY.Vector(point.x,point.y);
            },
            getXY:function(el) {
                if (document.documentElement.getBoundingClientRect) { // IE,FF3.0+,Opera9.5+
                    var box = el.getBoundingClientRect();
                    return new MY.Vector(box.left+this.getScrollXY().x,box.top+this.getScrollXY().y )
                } else {
                    ...
                    return new MY.Vector(pos[0],pos[1])
                }
            },
            getOffsetSize:function(ele){
                return new MY.Vector(ele.offsetWidth, ele.offsetHeight);
            },
            getMousePos:function(ev){
                if(ev.pageX || ev.pageY){

                    return new MY.Vector(ev.pageX,ev.pageY)
                }
                return new MY.Vector(
    ...
    CustomEvent.prototype={
        ...
        fire:function(param){
            ...
            for(var i=0,n=ev.length;i<n;i++){
                ev[i].func.call(this.handle,{event_param:ev[i].param,fire_param:param});
            }
        }
        /*END~!CustomEvent*/
    }
