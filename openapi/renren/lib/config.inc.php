...
$config->AUTHORIZEURL = $renren_config['token_url'];  //进行连接授权的地址，不需要修改
$config->ACCESSTOKENURL = $renren_config['access_url']; //获取access token的地址，不需要修改
$config->SESSIONKEYURL = $renren_config['session_url']; //获取session key的地址，不需要修改
$config->CALLBACK = $renren_config['callback_url']; //回调地址，注意和您申请的应用一致

$config->APIURL = $renren_config['api_url']; //RenRen网的API调用地址，不需要修改
$config->APIKey = $renren_config['api_key']; //你的API Key，请自行申请
$config->SecretKey = $renren_config['api_secret']; //你的API 密钥
